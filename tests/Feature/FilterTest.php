...
it('Run filter', function (?string $filter, string $result) {
    ...
    "Simple any filter but not expandable" => ["any(nonExistingModel, name eq 'Aqqo')", "select * from `test_models` limit 100 offset 0"],
    "Two filters with any filter" => ["name eq 'Aqqo' and any(related_model, name eq 'Aqqo')", "select * from `test_models` where ((`name` = 'Aqqo') and (exists (select * from `related_models` where `test_models`.`related_model_id` = `related_models`.`id` and `name` = 'Aqqo'))) limit 100 offset 0"],
    ...
    "Two filters with all filter but not expandable" => ["all(nonExistingModel, cost gt 10) and name eq 'Aqqo'", "select * from `test_models` where ((`name` = 'Aqqo')) limit 100 offset 0"],
]);